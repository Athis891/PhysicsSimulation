...
void Gizmos::addBlackAABBFilled(const glm::vec3& a_center,
	const glm::vec3& a_rvExtents,
	const glm::vec4& a_fillColour,
	const glm::mat4* a_transform /* = nullptr */)
{
...
void Gizmos::addAABBFilledBorderless(const glm::vec3& a_center,
	const glm::vec3& a_rvExtents,
	const glm::vec4& a_fillColour,
	const glm::mat4* a_transform /* = nullptr */)
{
...
void Gizmos::addBlackSphereFilled(const glm::vec3& a_center, float a_radius, int a_rows, int a_columns, const glm::vec4& a_fillColour,
	const glm::mat4* a_transform /*= nullptr*/, float a_longMin /*= 0.f*/, float a_longMax /*= 360*/,
	float a_latMin /*= -90*/, float a_latMax /*= 90*/)
{
	float inverseRadius = 1 / a_radius;
	...
	for (int face = 0; face < (a_rows)*(a_columns); ++face)
	{
		...
		addLine(a_center + v4Array[face], a_center + v4Array[face + a_columns], glm::vec4(0.f, 0.f, 0.f, 1.f), glm::vec4(0.f, 0.f, 0.f, 1.f));

		if (face % a_columns == 0 && longitudinalRange < (glm::pi<float>() * 2))
		{
			continue;
		}
		addLine(a_center + v4Array[iNextFace + a_columns], a_center + v4Array[face + a_columns], glm::vec4(0.f, 0.f, 0.f, 0.f), glm::vec4(0.f, 0.f, 0.f, 0.f));
