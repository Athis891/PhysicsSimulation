...
void ParticleFluidEmitter::releaseAll()
{
...
void ParticleFluidEmitter::renderParticles(glm::vec4 color,bool borderless)
{
	...
	if (fd)
	{
		...
		for (unsigned i = 0; i < fd->validParticleRange; ++i, ++flagsIt, ++positionIt,++densityIt)
		{
			if (*flagsIt & PxParticleFlag::eVALID)
			{
				...
				if (borderless)
				{
					Gizmos::addAABBFilledBorderless(pos, glm::vec3(.12, .12, .12), color);
				}
				else
				{
					Gizmos::addAABBFilled(pos, glm::vec3(.12, .12, .12), color);
				}
